...
系统采用**智能仓库对比**策略，基于阿里云仓库实际状态进行同步：

##### 核心特性
- **🔍 仓库检测**: 实时查询阿里云容器镜像服务API
- **📊 智能对比**: 对比配置文件与实际仓库状态
- **⚡ 按需同步**: 只同步缺失的镜像，跳过已存在的
- **🎯 精准识别**: 支持多架构、重名镜像的智能处理
...
### Q: 如何强制同步所有镜像？
A: 有几种方式：
1. **手动触发**: 在 GitHub Actions 页面选择 "Force sync: true" 选项
2. **定时检查**: 系统每天自动检查并同步缺失镜像
3. **删除仓库**: 手动删除阿里云上的特定仓库后重新同步
